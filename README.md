...
For marketplace compatibility, a paid bare `GET /mcp` or empty `POST /mcp`
replay returns a compact `200` capability document immediately. The x402
discovery metadata for `POST /mcp` also carries a valid default MCP initialize
request. Standard MCP initialize and session-bound requests continue through the
Streamable HTTP transport normally. Settlement uses the SDK's asynchronous mode
so the resource response is not held while waiting for an X Layer block
confirmation.
